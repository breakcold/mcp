...
| `assets/breakcold-mascot.webp`    | The Breakcold bear mascot (with rainbow VR goggles)         | Hero callout in reports, with a speech bubble           |
| `assets/report-template.html`     | A fully populated Breakcold-branded HTML report             | Chassis for Action 3 reports — see `report-design-guide.md` |

When generating reports or any branded visual, **reference these directly** if the delivery surface supports relative paths. Inline the SVG XML or base64-encode the mascot when delivering inline (Claude artifacts, ChatGPT canvas, email body).

Do **not** approximate the mark with a "blue square with white Br text" — the real SVG is right there. Same for the mascot: don't generate an AI-image of "a friendly bear" — use the canonical asset.
