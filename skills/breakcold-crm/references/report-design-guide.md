...
5. The mascot image at `assets/breakcold-mascot.webp` and the SVG mark in the footer come from the assets folder — keep the paths relative if the user will open the file locally, or inline as base64 if delivering inline in chat.
6. Update the date range, the workspace name (subtle, in the kicker), and the footer source line.

## When delivering inline (no file)

If the agent's runtime renders HTML inline (Claude artifacts, ChatGPT canvas, etc.), inline the SVG logos and base64-encode the mascot image. The CSS is self-contained — paste the `<style>` block, then the module HTML in order.
